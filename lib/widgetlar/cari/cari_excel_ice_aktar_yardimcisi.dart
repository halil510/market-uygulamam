// lib/widgetlar/cari/cari_excel_ice_aktar_yardimcisi.dart
// Tek sorumluluk: Excel'den cari içe aktarma akışını (dosya seç → işle →
...
class CariExcelIceAktarYardimcisi {
  /// Excel dosyası seçtirir, carileri içe aktarır ve sonucu dialog ile
  ...
  static Future<void> iceAktar(BuildContext context, {required VoidCallback onTamamlandi}) async {
    try {
      ...
      CariIceAktarSonuc? sonuc;
      try {
        sonuc = await ExcelServisi().exceldenCarileriBytesIceriAl(fileBytes);
      } catch (e) {
  ...
  static void _sonucDialog(BuildContext context, CariIceAktarSonuc sonuc) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          _sonucSatiri('Eklenen Cari', sonuc.eklenen, Colors.green),
          _sonucSatiri('Güncellenen Cari', sonuc.guncellenen, Colors.blue),
          if (sonuc.acilisBakiyesiYazilan > 0)
            _sonucSatiri('Açılış Bakiyesi Yazılan', sonuc.acilisBakiyesiYazilan, Colors.orange),
  ...
  static void _hataDialog(BuildContext context, CariIceAktarSonuc? sonuc) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        ...
        content: ConstrainedBox(
          ...
          child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Hiç cari eklenemedi/güncellenemedi.'),
