      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(width: 44, height: 44,
            decoration: BoxDecoration(color: const Color(0x1AE65100), borderRadius: BorderRadius.circular(12)),
            child: const Icon(Icons.inventory_2_outlined, color: _orange, size: 22)),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(urun.urunAdi, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: textD)),
            Text(urun.barkod ?? '', style: TextStyle(fontSize: 12, color: textL)),
          ])),
          IconButton(icon: Icon(Icons.close, color: textL), onPressed: onSifirla),
        ]),
        const Divider(height: 24),
        // Fiyat / Stok
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Normal Satış Fiyatı', style: TextStyle(fontSize: 11, color: textL)),
            Text(ParaUtils.formatla(urun.satisFiyati),
                style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: textD)),
          ])),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Mevcut Stok', style: TextStyle(fontSize: 11, color: textL)),
        ...
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(mainAxisSize: MainAxisSize.min, children: [
              GestureDetector(
                onTap: () { fiyatCtrl.text = urun.satisFiyat.toStringAsFixed(2); onDegisti(); },
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
                      color: _green.withAlpha(25),
                      borderRadius: BorderRadius.circular(6)),
                  child: Text('Satış: ${urun.satisFiyat.toStringAsFixed(2)}',
                      style: TextStyle(fontSize: 10, color: _green, fontWeight: FontWeight.w600)),
                ),
              ),
              const SizedBox(width: 6),
              GestureDetector(
                onTap: () { fiyatCtrl.text = urun.alisFiyat.toStringAsFixed(2); onDegisti(); },
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: const Color(0x0FE65100), borderRadius: BorderRadius.circular(12)),
          child: Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Ara Toplam:', style: TextStyle(fontSize: 12, color: textL)),
              Text(ParaUtils.formatla(_araToplam), style: TextStyle(fontSize: 13, color: textL)),
            ]),
            if (_iskonto > 0) ...[
              const SizedBox(height: 4),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text('İskonto (${iskontoCtrl.text}%):', style: const TextStyle(fontSize: 12, color: Colors.orange)),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('İade Toplam:', style: TextStyle(fontSize: 13, color: textL)),
