...
class BekleyenSiparisKalemGirdi {
  ...
  final double miktar; // kullanıcının girdiği miktar (birim cinsinden, ör. "3 Paket")
  ...
  const BekleyenSiparisKalemGirdi({
    required this.urunId, required this.urunAdi, required this.birimAdi,
    required this.birimCarpani, required this.miktar, required this.birimFiyat,
    required this.alisFiyat, this.iskontoOran = 0, this.kdvOran = 18,
  });
...
class BekleyenSiparisDeposu {
  Future<int> siparisOlustur({
    ...
    final araToplam = kalemler.fold(0.0, (s, k) => s + k.toplamMiktar * k.birimFiyat);
    ...
    final satir = await db.query('bekleyen_siparisler', where: 'id = ?', whereArgs: [siparisId], limit: 1);
    if (satir.isNotEmpty) BulutManager().upsert('bekleyen_siparisler', Map<String, dynamic>.from(satir.first));
    final kalemSatirlari = await db.query('bekleyen_siparis_kalem', where: 'siparis_id = ?', whereArgs: [siparisId]);
    for (final k in kalemSatirlari) {
      BulutManager().upsert('bekleyen_siparis_kalem', Map<String, dynamic>.from(k));
    }
    return siparisId;
  }

  Future<List<Map<String, dynamic>>> bekleyenSiparisleriGetir({int? cariId, String durum = 'bekliyor'}) async {
    final db = await Veritabani().db;
    final where = <String>['s.durum = ?', 's.deleted_at IS NULL'];
    final args = <Object?>[durum];
    if (cariId != null) { where.add('s.cari_id = ?'); args.add(cariId); }
    return db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> siparisKalemleriGetir(int siparisId) async {
    final db = await Veritabani().db;
    return db.query('bekleyen_siparis_kalem', where: 'siparis_id = ?', whereArgs: [siparisId]);
  }
  ...
  Future<void> iptalEt(int siparisId) async {
    final db = await Veritabani().db;
    final now = DateTime.now().toIso8601String();
    await db.update('bekleyen_siparisler', {'durum': 'iptal', 'last_updated': now},
  ...
  Future<int> onaylaVeSatisaCevir({
    ...
    final genelToplam = kalemSatirlari.fold(0.0, (s, k) => s + (k['toplam_tutar'] as num).toDouble());
    ...
    final satisKalemler = kalemSatirlari.map((k) {
      ...
      return SatisKalemModel(
        ...
        urunAdi: '${k['urun_adi']} (${k['birim_adi']} x${(k['miktar'] as num).toStringAsFixed((k['miktar'] as num) == (k['miktar'] as num).roundToDouble() ? 0 : 1)})',
        miktar: toplamMiktar,
    ...
    await db.transaction((txn) async {
      ...
      for (final k in kalemSatirlari) {
        ...
        await StokDeposu().stokDusTxn(txn, gid,
          urunId: urunId,
      ...
      cariGlobalId = await CariDeposu().hareketEkleTxn(txn, CariHareketModel(
        cariId: cari.id!,
        tarih: tarih,
        fisTipi: 'Toptan Satış (Sipariş)',
        fisId: satisId,
    ...
    try {
      ...
      for (final k in kalemSatirlari) {
        final urunId = k['urun_id'] as int;
        final gid = stokHareketGidleri[urunId];
      ...
      final cariSatir = await db.query('cari', where: 'id = ?', whereArgs: [cari.id], limit: 1);
