...
extension _HizliTabExt on _IadeEkraniState {
  Future<void> _hizliBarkod() async {
    ...
    if (urun != null) {
      ...
      if (_hizliMap.containsKey(urun.id)) {
        ...
        _msg('${urun.urunAdi} — toplam adet: ${_hizliMap[urun.id]!.adet}', err: false);
      } else {
  ...
  Future<void> _hizliKaydet() async {
    ...
    try {
      ...
      final toplamIade = _hizliMap.values.fold<double>(0.0, (s, i) => s + i.adet * i.urun.satisFiyati);
      ...
      await db.transaction((txn) async {
        ...
        for (final item in _hizliMap.values) {
          ...
          await txn.insert('iade_kalem', {
            ...
            'iade_id': iadeId, 'urun_id': item.urun.id,
            'urun_adi': item.urun.urunAdi, 'miktar': item.adet.toDouble(),
            'birim_fiyat': fiyat, 'toplam': toplam,
          });
          ...
          if (urunRows.isNotEmpty) {
            ...
            await txn.update('urunler', {'stok': onceki + item.adet, 'last_updated': now},
      ...
      try {
        ...
        for (final item in _hizliMap.values) {
          final kalemSatir = await db.query('iade_kalem',
              where: 'iade_id = ? AND urun_id = ?', whereArgs: [iadeId, item.urun.id], limit: 1);
          if (kalemSatir.isNotEmpty) BulutManager().upsert('iade_kalem', Map<String, dynamic>.from(kalemSatir.first));
          final urunSatir = await db.query('urunler', where: 'id = ?', whereArgs: [item.urun.id], limit: 1);
          ...
          final stokSatir = await db.query('stok_hareket',
              ...
              whereArgs: [iadeId, 'iade', item.urun.id], orderBy: 'id DESC', limit: 1);
          if (stokSatir.isNotEmpty) BulutManager().upsert('stok_hareket', Map<String, dynamic>.from(stokSatir.first));
          _iadeListesi.add({
            'tarih': DateTime.now(), 'urun_adi': item.urun.urunAdi,
            'barkod': item.urun.barkod ?? '', 'miktar': item.adet.toDouble(),
            'birim_fiyat': item.urun.satisFiyati, 'toplam_tutar': item.adet * item.urun.satisFiyati,
            'musteri_adi': _secilenCari?.unvan ?? 'Perakende', 'aciklama': 'Toplu iade - $fisNo',
          });
        }
        if (toplamIade > 0) {
  ...
  Widget _hizliTab() => Column(children: [
    Padding(padding: const EdgeInsets.all(12), child: Row(children: [
      Expanded(child: FilledButton.icon(
        ...
        style: FilledButton.styleFrom(backgroundColor: _R.primary, foregroundColor: Colors.white),
    ...
    if (_hizliMap.isEmpty)
      Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(color: Color.fromARGB(20, _R.blue.red, _R.blue.green, _R.blue.blue), shape: BoxShape.circle),
          child: const Icon(Icons.add_shopping_cart, size: 52, color: _R.blue)),
        const SizedBox(height: 16),
        const Text('Hızlı iade listesi boş', style: TextStyle(fontSize: 15, color: _R.textL)),
        const SizedBox(height: 6),
        const Text('Barkod okutarak veya arama yaparak ürün ekleyin',
            style: TextStyle(fontSize: 12, color: _R.textL), textAlign: TextAlign.center),
      ])))
    else
      Expanded(child: Column(children: [
        Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('${_hizliMap.length} çeşit ürün', style: const TextStyle(fontWeight: FontWeight.w600)),
            TextButton.icon(icon: const Icon(Icons.delete_sweep, size: 16), label: const Text('Temizle'),
                onPressed: () { _hizliMap.clear(); if (mounted) setState(() {}); }),
          ])),
        Expanded(child: ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return Container(margin: const EdgeInsets.only(bottom: 8), decoration: BoxDecoration(color: TsRenk.kart(context), borderRadius: BorderRadius.circular(12), border: Border.all(color: TsRenk.ayirac(context))), child: Padding(
              padding: const EdgeInsets.all(12),
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(item.urun.urunAdi, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                  Text(item.urun.barkod ?? '', style: const TextStyle(fontSize: 11, color: _R.textL)),
                ])),
                Row(children: [
                  IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 32, minHeight: 32),
                    icon: const Icon(Icons.remove_circle_outline, size: 22),
                    onPressed: () => setState(() {
                      if (item.adet <= 1) _hizliMap.remove(item.urun.id);
                      else item.adet--;
                    })),
                  Text('${item.adet}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 32, minHeight: 32),
