...
class IadeEkrani extends ConsumerStatefulWidget {
  ...
  const IadeEkrani({super.key, this.baslangicUrunleri, this.otomatikKapat = false});

  @override
  ConsumerState<IadeEkrani> createState() => _IadeEkraniState();
}

class _IadeEkraniState extends ConsumerState<IadeEkrani> with TickerProviderStateMixin {
  ...
  int? _duzenlemeModu_iadeId;       // null = yeni iade, int = mevcut iade üzerinde çalışıyoruz
  ...
  void initState() {
    ...
    _tab.addListener(() { if (_tab.index == 3) _gecmisYukle(); });
  ...
  Future<void> _verileriYukle() async {
    try {  
      ...
      final results = await Future.wait([_urunDepo.tumunuGetir(), _cariDepo.tumunuGetir()]);
      if (!mounted) return;
      ...
      if (mounted) BildirimServisi.hata(context, 'Veriler yüklenemedi, lütfen tekrar deneyin');
    }
  }
  ...
  void _aramaDebounce() {
    ...
    if (q.isEmpty) { _aramaListesi = []; if (mounted) setState(() {}); return; }
  ...
  Future<void> _yeniFisNoOlustur() async {
    try {
      final no = await Veritabani().fisNoUret('iade', subeId: AktifSubeServisi().subeId ?? 1);
      if (mounted) setState(() { _fisNo = ''; _oturumFisNo = no; });
    } catch (_) {
      _oturumFisNo = 'IAD-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
    }
  }
  ...
  Future<void> _iadeIptal(Map<String, dynamic> iade) async {
    final onay = await showDialog<bool>(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        ...
        content: Text('${iade['fis_no']} numaralı iade iptal edilsin mi?\n\nStok geri alınacak.'),
        actions: [
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        final kasaRows = await txn.query('kasa_hareketleri',
            where: 'referans_id = ? AND referans_turu = ?', whereArgs: [iadeId, 'iade'], limit: 1);
        ...
        if (cariHareketRows.isNotEmpty) {
          ...
          final eskiBorc = (cariHareketRows.first['borc'] as num?)?.toDouble() ?? 0;
          final eskiAlacak = (cariHareketRows.first['alacak'] as num?)?.toDouble() ?? 0;
          if (cariId != null) {
      ...
      try {
        ...
        if (kasaHareketGid != null) {
          final kasaSatir = await db.query('kasa_hareketleri', where: 'global_id = ?', whereArgs: [kasaHareketGid], limit: 1);
        ...
        if (cariHareketGid != null) {
          final cariHareketSatir = await db.query('cari_hareket', where: 'global_id = ?', whereArgs: [cariHareketGid], limit: 1);
  ...
  Future<void> _kaydet() async {
    ...
    if (_duzenlemeModu_iadeId != null) {
      ...
      await _duzenlemeModu_kalemEkle(fiyat, isk, _miktar * fiyat * (isk/100), fiyat * (1-isk/100), toplam);
      return;
    }
    ...
    try {
      ...
      final isk     = ParaUtils.sayiCoz(_iskontoCtrl.text) ?? 0;
      final toplam  = _miktar * fiyat * (1 - isk / 100);
      final neden   = _aciklamaCtrl.text.trim().isEmpty ? 'Iade' : _aciklamaCtrl.text.trim();
      final db      = await Veritabani().db;

      // Fiş no transaction dışında (sequence güncelleme ayrı transaction gerektirir)
      if (_oturumFisNo.isEmpty) _oturumFisNo = await Veritabani().fisNoUret('iade', subeId: AktifSubeServisi().subeId ?? 1);
      ...
      await db.transaction((txn) async {
        ...
        // 4. Kasa — KasaDeposu ile AYNI güvenli bakiye sorgusu kullanılıyor:
        // deleted_at IS NULL + tarih DESC sıralaması. 'ORDER BY id DESC'
        // TEK BAŞINA yanlış olabilirdi: başka bir cihazdan senkronla gelen,
        // daha ESKİ tarihli ama bu cihazda YENİ id alan bir kayıt varsa
        // (çoklu cihaz senkronunda olağan bir durum), yanlış bakiyeyi
        // "son bakiye" sanıp üzerine yazardı.
      ...
      try {
        ...
        final kalemSatir = await db.query('iade_kalem',
            where: 'iade_id = ? AND urun_id = ?', whereArgs: [iadeId, _secilenUrun!.id], limit: 1);
      ...
      final mevcutIdx = _iadeListesi.indexWhere((x) => x['urun_id'] == _secilenUrun!.id);
      if (mevcutIdx != -1) {
      ...
      if (si != -1) _tumUrunler[si] = _tumUrunler[si].copyWith(stok: _tumUrunler[si].stok + _miktar);
  ...
  Future<void> _excel() async {
    if (_iadeListesi.isEmpty) { _msg('İade kaydı yok', err: true); return; }
    try {
      final path = await _excelSrv.iadelerExcelEAktar(_iadeListesi);
      await _excelSrv.paylasExcel(path);
    } catch (e) { _msg('Excel hatası: $e', err: true); }
  }
    ...
    return Scaffold(
      ...
      appBar: TsAppBar(
        baslikWidget: _duzenlemeModu_iadeId != null
            ? Text('İade: ${_duzenlemeModu_fisNo ?? ''}', style: const TextStyle(fontSize: 15))
            : const Text('İade İşlemleri'),
        aksiyonlar: [
          if (_duzenlemeModu_iadeId != null)
            TextButton.icon(
              icon: const Icon(Icons.add_circle_outline, color: Colors.white, size: 16),
              label: const Text('Yeni', style: TextStyle(color: Colors.white, fontSize: 11)),
              onPressed: _yeniIadeBaslat,
            ),
          IconButton(icon: Image.asset("assets/images/excel_icon.png", width: 22, height: 22, errorBuilder: (_, __, ___) => const Icon(Icons.table_chart)), tooltip: 'Excel', onPressed: _excel),
          if (_secilenCari != null)
            Chip(label: Text(_secilenCari!.unvan, style: const TextStyle(fontSize: 11)),
              deleteIcon: const Icon(Icons.close, size: 14),
              onDeleted: () { _secilenCari = null; if (mounted) setState(() {}); }),
            ...
            onSelected: (v) async {
              if (v == 'musteri') {
                final secilen = await showDialog<CariModel?>(context: context,
                  builder: (ctx) => CariSecDialog(cariler: _cariler));
                if (secilen != null) { _secilenCari = secilen; if (mounted) setState(() {}); }
              }
            },
            itemBuilder: (_) => [
              const PopupMenuItem(value: 'musteri',
                  child: ListTile(dense: true, leading: Icon(Icons.person, color: AppRenkler.primary), title: Text('Müşteri Seç'))),
            ],
          ),
        ],
        alt: TabBar(
      ...
      if (_secilenUrun != null) ...[
        ...
        SizedBox(width: double.infinity, height: 52,
          child: FilledButton.icon(
            ...
            style: FilledButton.styleFrom(backgroundColor: _R.orange, foregroundColor: Colors.white),
            icon: _yukleniyor
                ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                : const Icon(Icons.assignment_return),
            label: const Text('İade Et', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ),
        ),
      ] else
        _bosEkran(),
  ...
  Widget _aramaKutusu() => IadeAramaKutusu(
    ...
    onTemizle: () { _aramaCtrl.clear(); _aramaListesi = []; if (mounted) setState(() {}); },
    onBarkod: _barkodOku,
  );
  ...
  Widget _eskiUrunFormuGovdesi() => Container(
    ...
    decoration: BoxDecoration(color: _R.card, borderRadius: BorderRadius.circular(16),
      boxShadow: const [BoxShadow(color: _R.shadow, blurRadius: 8, offset: Offset(0, 2))]),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Container(width: 44, height: 44,
          decoration: BoxDecoration(color: Color.fromARGB(26, _R.orange.red, _R.orange.green, _R.orange.blue), borderRadius: BorderRadius.circular(12)),
          child: const Icon(Icons.inventory_2_outlined, color: _R.orange, size: 22)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(_secilenUrun!.urunAdi, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _R.textD)),
          Text(_secilenUrun!.barkod ?? '', style: const TextStyle(fontSize: 12, color: _R.textL)),
        ])),
        IconButton(icon: const Icon(Icons.close, color: _R.textL), onPressed: _formSifirla),
      ]),
      const Divider(height: 24),
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Normal Satış Fiyatı', style: TextStyle(fontSize: 11, color: _R.textL)),
          Text(ParaUtils.formatla(_secilenUrun!.satisFiyat),
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: _R.textD)),
        ])),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Mevcut Stok', style: TextStyle(fontSize: 11, color: _R.textL)),
          Text(_secilenUrun!.stokMiktari.toStringAsFixed(0),
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold,
                  color: _secilenUrun!.stokMiktari <= 0 ? _R.red : _R.green)),
        ])),
      ]),
      const SizedBox(height: 16),
      Row(children: [
      ...
      Container(padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(color: Color.fromARGB(15, _R.orange.red, _R.orange.green, _R.orange.blue), borderRadius: BorderRadius.circular(12)),
        child: Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Ara Toplam:', style: TextStyle(fontSize: 12, color: _R.textL)),
            Text(ParaUtils.formatla(_miktar * (ParaUtils.sayiCoz(_fiyatCtrl.text) ?? _orijinalFiyat)),
                style: const TextStyle(fontSize: 13, color: _R.textL)),
          ]),
          if ((ParaUtils.sayiCoz(_iskontoCtrl.text) ?? 0) > 0) ...[
            const SizedBox(height: 4),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('İskonto (${_iskontoCtrl.text}%):', style: const TextStyle(fontSize: 12, color: Colors.orange)),
              Text('- ${ParaUtils.formatla(_miktar * (ParaUtils.sayiCoz(_fiyatCtrl.text) ?? _orijinalFiyat) * (ParaUtils.sayiCoz(_iskontoCtrl.text) ?? 0) / 100)}',
                  style: const TextStyle(fontSize: 12, color: Colors.orange)),
            ]),
          ],
          const Divider(height: 8),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('İade Toplam:', style: TextStyle(fontSize: 13, color: _R.textL)),
            Text(ParaUtils.formatla(_miktar * (ParaUtils.sayiCoz(_fiyatCtrl.text) ?? _orijinalFiyat) * (1 - (ParaUtils.sayiCoz(_iskontoCtrl.text) ?? 0) / 100)),
  ...
  Widget _bosEkran() => Center(child: Padding(
    padding: const EdgeInsets.symmetric(vertical: 48),
    child: Column(children: [
      Opacity(
        opacity: 0.3,
        child: Image.asset('assets/images/empty_box.png', height: 80,
          errorBuilder: (_, __, ___) => const Icon(Icons.assignment_return, size: 52, color: _R.orange)),
      ),
    ]),
  ));
