...
class _UrunListeEkraniState extends ConsumerState<UrunListeEkrani> {
  ...
  void _hizliBilgiGoster(UrunModel u) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _bilgiSatir('Satış Fiyatı', ParaUtils.formatla(u.satisFiyati)),
        ...
        _bilgiSatir('Kar Oranı', '%${u.karOrani.toStringAsFixed(1)}'),
      ]),
      actions: [
              const BulutDurumIkonu(),
        TextButton(onPressed: () { Navigator.pop(ctx); context.push('/urun/detay/${u.id}'); }, child: const Text('Detay')),
  ...
  Widget _bilgiSatir(String etiket, String deger) => Padding(
    ...
    child: Row(children: [
      SizedBox(width: 90, child: Text(etiket, style: TextStyle(fontSize: 12, color: TsRenk.metinIkincil(context)))),
      Expanded(child: Text(deger, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600))),
    ]));
  ...
  void _aramaChanged() {
    ...
    _araDebounce = Timer(const Duration(milliseconds: 300), () {
      ref.read(urunFiltresiProvider.notifier)
  ...
  Future<void> _excelAktar({bool sutunSec = false}) async {
    ...
    try {
      ...
      final path = '${dir.path}/urunler_${DateTime.now().millisecondsSinceEpoch}.xlsx';
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Align(alignment: Alignment.centerLeft,
                  child: Text('Sıralama', style: TextStyle(
                      fontWeight: FontWeight.w600, color: context.textSecondary))),
              const SizedBox(height: 8),
              Wrap(spacing: 8, runSpacing: 8, children: [
                for (final (deger, etiket) in siralamaSecenekleri)
                  ChoiceChip(
                    label: Text(etiket, style: TextStyle(fontSize: 12,
                        color: tmpSiralama == deger ? Colors.white : context.textPrimary)),
              ...
              if (gruplar.isNotEmpty) ...[
                Divider(height: 24, color: context.dividerColor),
                Align(alignment: Alignment.centerLeft,
                    child: Text('Grup', style: TextStyle(
                        fontWeight: FontWeight.w600, color: context.textSecondary))),
                const SizedBox(height: 8),
                DropdownButtonFormField<String?>(
                  ...
                  hint: Text('Tüm Gruplar', style: TextStyle(color: context.textHint)),
                  items: [
                    DropdownMenuItem(value: null, child: Text('Tüm Gruplar',
                        style: TextStyle(color: context.textPrimary))),
  ...
  Widget build(BuildContext context) {
    ...
    final filtreAktif = filtre.sadecKritikler == true ||
        filtre.grupFiltre != null || filtre.siralama != 'isim';
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: durum.secimModu
            ? AppBar(
                ...
                leading: IconButton(
                  ...
                  onPressed: () => ref.read(urunlerProvider.notifier).secimTemizle(),
                ),
                title: Text('${durum.seciliIds.length} seçili'),
                actions: [
                  IconButton(
                    icon: const Icon(Icons.select_all),
                    tooltip: 'Tümünü Seç',
                    onPressed: () => ref.read(urunlerProvider.notifier).tumunuSec(),
                  ),
                  IconButton(
          ...
          actions: [
            ...
            IconButton(
              icon: Icon(_izgara ? Icons.list : Icons.grid_view, size: 22),
            ...
            PopupMenuButton<String>(
              ...
              onSelected: (v) {
                ...
                if (v == 'excel_ice') ExcelIceAktarYardimcisi.iceAktar(context,
                    onTamamlandi: () => ref.read(urunlerProvider.notifier).yukle(sifirla: true));
                if (v == 'toplu_fiyat')
                  context.push('/urun/toplu-fiyat');
              },
              itemBuilder: (_) => const [
                PopupMenuItem(
                  value: 'excel_ice',
                ...
                PopupMenuItem(
                  ...
                  child: ListTile(dense: true,
                    ...
                    title: Text('Excel Aktar', style: TextStyle(fontSize: 13)))),
                PopupMenuItem(
                  value: 'toplu_fiyat',
                  child: ListTile(dense: true,
                    leading: Icon(Icons.price_change, size: 18, color: AppRenkler.primary),
                    title: Text('Toplu Fiyat', style: TextStyle(fontSize: 13)))),
              ],
            ),
          ],
        ),
                    ...
                    suffixIcon: Row(mainAxisSize: MainAxisSize.min, children: [
                      if (_araCtrl.text.isNotEmpty)
                        IconButton(icon: const Icon(Icons.clear, size: 18),
                          onPressed: () {
                            _araCtrl.clear();
                            ref.read(urunFiltresiProvider.notifier).aramaGuncelle('');
                          }),
                      IconButton(icon: Icon(Icons.qr_code_scanner_outlined, size: 20, color: context.textSecondary),
                        tooltip: 'Barkod Tara',
                        onPressed: () async {
                          final b = await BarkodServisi().barkodTara(context);
                          if (b != null && mounted) {
                            _araCtrl.text = b;
                            ref.read(urunFiltresiProvider.notifier).aramaGuncelle(b);
                          }
                        }),
                    ]),
        ...
        floatingActionButton: TsYetkili(child: FloatingActionButton(
          backgroundColor: AppRenkler.primary,
          foregroundColor: Colors.white,
          tooltip: 'Ürün Ekle',
          onPressed: () async {
  ...
  Widget _listeView(List<UrunModel> urunler, UrunListeDurum durum) => ListView.builder(
    ...
    itemBuilder: (_, i) {
      ...
      final stokRenk = u.stok <= 0
          ? Colors.red
          : u.kritikStok ? Colors.orange : Colors.green;

      final kart = RepaintBoundary(
        child: TsKart(
          padding: const EdgeInsets.fromLTRB(12, 13, 12, 13),
          onLongPress: () => ref.read(urunlerProvider.notifier).secimToggle(u.id!),
          onTap: () {
            if (durum.secimModu) {
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        colors: [Color.fromARGB(46, stokRenk.red, stokRenk.green, stokRenk.blue), Color.fromARGB(15, stokRenk.red, stokRenk.green, stokRenk.blue)],
                        begin: Alignment.topLeft, end: Alignment.bottomRight),
                      borderRadius: BorderRadius.circular(14),
                      border: Border.all(color: Color.fromARGB(64, stokRenk.red, stokRenk.green, stokRenk.blue)),
                      image: u.resimYolu != null && u.resimYolu!.isNotEmpty && File(u.resimYolu!).existsSync()
                          ? DecorationImage(image: FileImage(File(u.resimYolu!)), fit: BoxFit.cover)
                          : null,
                    ),
                    child: (u.resimYolu == null || u.resimYolu!.isEmpty || !File(u.resimYolu!).existsSync())
                        ? Center(child: Text(u.urunAdi[0].toUpperCase(),
                            style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: stokRenk)))
                        : null,
                  ),
                ),
                const SizedBox(width: 12),
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: Color.fromARGB(31, stokRenk.red, stokRenk.green, stokRenk.blue),
                        borderRadius: BorderRadius.circular(8)),
                      child: Text(
                        '${u.stok.toStringAsFixed(u.stok == u.stok.roundToDouble() ? 0 : 1)} ${u.birimAdi}',
                        style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w800, color: stokRenk)),
                    ),
                    const SizedBox(width: 8),
                    Flexible(child: Text('Alış: ${ParaUtils.formatla(u.alisFiyatKdvDahil)}',
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: Colors.blue.shade50,
                          borderRadius: BorderRadius.circular(8)),
                        child: const Icon(Icons.info_outline, size: 19, color: Colors.blue),
                      ),
                    ),
                    const SizedBox(width: 10),
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(8)),
                        child: Icon(Icons.linear_scale, size: 19,
        ...
        if (durum.secimModu)
          Positioned(
            ...
            child: GestureDetector(
              onTap: () => ref.read(urunlerProvider.notifier).secimToggle(u.id!),
              child: Container(
                width: 24, height: 24,
                decoration: BoxDecoration(
                  color: durum.seciliIds.contains(u.id!)
                      ? AppRenkler.primary : Colors.white,
                  shape: BoxShape.circle,
                  border: Border.all(
                    color: durum.seciliIds.contains(u.id!)
                        ? AppRenkler.primary : context.textSecondary,
                    width: 1.5),
  ...
  Widget _izgaraView(List<UrunModel> urunler, UrunListeDurum durum) => GridView.builder(
    controller: _scrollCtrl,
    padding: const EdgeInsets.fromLTRB(8, 4, 8, 80),
    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount:   TsResponsive.izgaraKolonSayisi(context, telefon: 2, tablet: 4, genis: 5),
        ...
        child: TsKart(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Row(
                ...
                GestureDetector(
                  ...
                  child: Container(
                    width: 48, height: 48,
                    decoration: BoxDecoration(
                        color: Color.fromARGB(26, stokRenk.red, stokRenk.green, stokRenk.blue),
                        borderRadius: BorderRadius.circular(12),
                ...
                if (u.barkod != null && u.barkod!.isNotEmpty)
                  IconButton(
                    ...
                    onPressed: () => _barkodGoster(u.barkod!, u.urunAdi),
              ...
              Container(
                ...
                child: Text(
                    '${u.stok.toStringAsFixed(0)} ${u.birimAdi}',
                    style: TextStyle(
                        fontSize: 12,
...
class _IkonButon extends StatelessWidget {
  ...
  const _IkonButon({required this.icon, required this.renk, required this.bg, required this.onTap});
  @override
